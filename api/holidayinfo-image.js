/**
 * Proxy endpoint pro Holidayinfo last_image API
 *
 * Bezpečně volá Holidayinfo API bez odhalení download kódu ve frontendu
 *
 * Parametry:
...
// DŮLEŽITÉ: Download kód by měl být v environment variables
// Pro development můžeš použít hardcoded hodnotu, ale pro production
// nastav HOLIDAYINFO_DC v Vercel environment variables
...
export default async function handler(req, res) {
  ...
  const targetUrl = `${HOLIDAYINFO_BASE_URL}/loc_cams_lastimage.php?${params.toString()}`;

  try {
    const upstream = await fetch(targetUrl);

    if (!upstream.ok) {
      console.error(`Holidayinfo API error: ${upstream.status} for camid=${camid}`);
    ...
    // Cache hlavičky - obrázky se mění každých ~5 minut
    ...
    console.error("holidayinfo-image proxy error:", err);
